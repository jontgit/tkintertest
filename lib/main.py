...
# FIXED   - Yield statment doesn't account for whitespace
# FIXED   - When changing known jobs, joblist doesn't update properly
...
class Application(TkinterDnD.Tk):
    ...
    def __init__(self):
        ...
        """self.file_menu = tk.Menu(self.toolbar, tearoff="off", name="!file_menu")
        self.toolbar.add_cascade(label="File", menu=self.file_menu)
        self.file_menu.add_command(label="Import File           ", font=("Consolas", 10), command=self.request_file)
        self.file_menu.add_command(label="Export File           ", font=("Consolas", 10), command=self.request_file)
        ...
        self.toolbar.add_cascade(label="Scripts", font=("Consolas", 10), menu=self.scripts_menu)
        self.scripts_menu.add_command(label="Create New         Ctrl+N", font=("Consolas", 10), command=self.editor_new_file)
        self.scripts_menu.add_command(label="Edit Current       Ctrl+E", font=("Consolas", 10), command=self.editor_edit_current_file)
        self.scripts_menu.add_command(label="Edit               Ctrl+R", font=("Consolas", 10), command=self.editor_edit_file)
        self.scripts_menu.add_command(label="Delete             Ctrl+D", font=("Consolas", 10), command=self.delete_script)
        
        # Settings Menu

        self.settings_menu = tk.Menu(self.toolbar, tearoff="off")
        self.toolbar.add_cascade(label="Settings", font=("Consolas", 10), menu=self.settings_menu)
        self.settings_menu.add_command(label="Preferences       Ctrl+P", font=("Consolas", 10), command=self.editor_new_file)
        self.settings_menu.add_command(label="Style             Ctrl+S", font=("Consolas", 10), command=self.editor_edit_current_file)
    ...
    def update_file_menu(self):
        ...
        self.file_menu = tk.Menu(self.toolbar, tearoff="off", name="!file_menu")
        self.toolbar.add_cascade(label="File", font=("Consolas", 10), menu=self.file_menu)
        self.file_menu.add_command(label="Import File           Ctrl+I", font=("Consolas", 10), command=self.request_file)
        ...
        for obj in self.toolbar.winfo_children():
            print(obj)
        ...
        if self.check_entry():
            ...
            for host in selection:
                ...
                self.manager_queue.put({
                    ...
                    "address" : self.device_data[host]['address'],
                    "index" : host,
                    "device_type" : self.device_types[self.device_selection_entry.get()],
                    ...
                    for entry in input_data:
                        ...
                        for key in entry.keys():
                            if key in ['hostname', 'DisplayName', 'Hostname']:
                                hostname = entry[key]
                            elif key in ['ipaddress', 'ip', 'Server External IP']:
                                ipaddress = entry[key]
                            else:
