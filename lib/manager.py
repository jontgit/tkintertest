...
class ManagerThread(Thread):
    def __init__(self, manager_queue, complete_queue, root, pause_event):
        """
        ...
        for i in range(CONCURRENT_SSH_LIMIT):
            self.worker_threads.append(WorkerThread(self.work_queue, complete_queue, root, pause_event).start())
