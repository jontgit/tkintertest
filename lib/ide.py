...
syntax = [
    ...
    {
        "regex" : r"(^|\s+)(in|for|while|return|break|import|if|else|elif|not)(?:\:)\s+",
        "tag" : "loop_statement",
        "fg" : "plum3"
    },
    {
        "regex" : r"(^|\s+)(in|for|while|return|break|import|if|not)\s+",
        "tag" : "loop_statement_1",
        "fg" : "plum3"
    },
    {
        "regex" : r"\.",
...
class LineCounter(tk.Canvas):
    ...
    def draw_linecount(self, line_count):
        ...
        for line in range(1, line_count):
            ...
            label.place(x=0, y=22*(line-1), width=40)
...
class Script():
    ...
    def run(self):
        ...
        self.file_menu.add_command(label="New           Crtl+N", font=("Consolas", 10), command=self.new_file)
        self.file_menu.add_command(label="Open          Crtl+O", font=("Consolas", 10), command=self.open_file)
        self.file_menu.add_command(label="Save          Crtl+S", font=("Consolas", 10), command=self.save)
        self.file_menu.add_command(label="Save As       Crtl+A", font=("Consolas", 10), command=self.save_as)
        self.file_menu.add_separator()
        self.file_menu.add_command(label="Exit          Escape", font=("Consolas", 10), command=self.exit)

        self.edit_menu = tk.Menu(self.toolbar, tearoff="off")
        self.toolbar.add_cascade(label="Edit", menu=self.edit_menu)
        self.edit_menu.add_command(label="Undo          Ctrl+Z", font=("Consolas", 10), command=self.undo)
        self.edit_menu.add_separator()
        self.edit_menu.add_command(label="Cut           Ctrl+X", font=("Consolas", 10), command=self.cut)
        self.edit_menu.add_command(label="Copy          Ctrl+C", font=("Consolas", 10), command=self.copy)
        self.edit_menu.add_command(label="Paste         Ctrl+V", font=("Consolas", 10), command=self.paste)
        self.edit_menu.add_command(label="Delete        Ctrl+D", font=("Consolas", 10), command=self.delete)
        self.edit_menu.add_separator()
        self.edit_menu.add_command(label="Find          Ctrl+F", font=("Consolas", 10), command=self.find)
    ...
    def save(self):
        ...
        print(repr(base_script))
        for line in base_script.split('\r'):
            if '\\n' in line or '\n' in line:
                print(line)
