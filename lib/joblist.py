...
class JobList(tk.Canvas):
    ...
    def _reset_job(self, index):
        ...
        if self.root.device_data[index]['address'] == "N/a":
    ...
    def _open_putty_thread(self):
        path = __file__[:-10].replace('\\','/')
        username = self.root.username.get()
        password = self.root.password.get()
        if username != "Username..." and password != "Password...":
            os.system(f"\"{path}/lib/putty/putty.exe\" {self.root.device_data[self.right_clicked_job]['address']} -l {username} -pw {password}")
        elif username != "Username...":
            os.system(f"\"{path}/lib/putty/putty.exe\" {self.root.device_data[self.right_clicked_job]['address']} -l {username}")
        else:
            os.system(f"\"{path}/lib/putty/putty.exe\" {self.root.device_data[self.right_clicked_job]['address']}")



    def _open_putty_session(self):
        threading.Thread(target=self._open_putty_thread, daemon=True).start()
